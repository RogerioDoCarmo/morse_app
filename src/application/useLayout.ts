...
import { fitsTwoColumns, isTabletWidth } from '@/core/domain/layout';

/** Which shape the app should take for the room it currently has. */
export function useLayout(): Readonly<{ tablet: boolean; twoColumns: boolean }> {
  ...
  return { tablet: isTabletWidth(width), twoColumns: fitsTwoColumns(width) };
}
